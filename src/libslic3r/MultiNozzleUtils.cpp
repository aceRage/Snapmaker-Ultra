...
{
    // Ultra: this post-remap print-time recalc is a GUI-only estimate that depends on
    // GCodeProcessorResult::filament_change_sequence and the AMS load/unload time tables
    // (get_ams_load_times/get_ams_unload_times) — neither of which this fork carries. Report
    // Unknown topology so estimate_mapped_print_time returns UnsupportedTopology and callers
    // fall back to the sliced total time. (Not needed by the dual-nozzle gcode path.)
    (void)slice_result;
    (void)printer_config;
    PrintTimeRecalcInput in;
    in.topology                     = Topology::Unknown;
