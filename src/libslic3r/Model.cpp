        ...
        if (result) {
            ...
            if (objFn) {
                ...
                if (!gltf_info.vertex_colors.empty()) {
                    // COLOR_0: one colour per vertex across every volume, so this becomes ordinary
                    // MMU painting, exactly like the OBJ vertex-colour path.
                    std::vector<unsigned char> vertex_filament_ids;
                    ...
                    // no dialog at all: stricter than OBJ, which always asks.
            ...
            // The geometry loaded; only the colour step can have failed here. Say so, rather than
            // letting the tail below throw the generic "Loading of a model file failed."
            if (!result && message.empty())
                message = _L("The colours in this glTF file could not be applied to the model.");
        }
    }
    else
...
// Paint one ModelVolume from a per-vertex filament id array; `vertex_filament_ids` addresses
// this volume's vertices and must have exactly `vertex_count` entries. Lifted unchanged out of
// Model::obj_import_vertex_color_deal so the OBJ path and the glTF multi-volume path share one
// implementation - including the split-triangle encoding for a face whose three corners land on
// three different filaments, which is the interesting part.
static bool paint_volume_from_vertex_colors(ModelVolume *volume, const unsigned char *vertex_filament_ids,
                                            size_t vertex_count)
{
    ...
    if (volume->mesh().its.vertices.size() != vertex_count) {
        return false;
    }
...
bool Model::obj_import_vertex_color_deal(const std::vector<unsigned char> &vertex_filament_ids, const unsigned char &first_extruder_id, Model *model)
{
    ...
    if (model->objects.size() == 1 ) {
        ...
        if (obj->volumes.size() == 1) {
            ...
            return paint_volume_from_vertex_colors(volume, vertex_filament_ids.data(), vertex_filament_ids.size());
        }
    }
    return false;
}

// Ultra (glTF): one filament id per ModelVolume of the single imported object. No painting - a
// glTF material is per primitive and a primitive is a volume, so the natural result is a per-part
// "extruder" config. Parts stay editable and nothing lands in mmu_segmentation_facets.
bool Model::import_volume_color_deal(const std::vector<unsigned char> &volume_filament_ids, const unsigned char &first_extruder_id, Model *model)
{
    if (volume_filament_ids.empty())
        return false;
    if (model->objects.size() != 1)
        return false;
    auto obj = model->objects[0];
    if (volume_filament_ids.size() != obj->volumes.size())
        return false;
    obj->config.set("extruder", first_extruder_id);
    for (size_t i = 0; i < obj->volumes.size(); i++) {
        const unsigned char id = volume_filament_ids[i];
        if (id <= 1)
            continue;   // 1 is the default; the OBJ appliers skip it for the same reason
        obj->volumes[i]->config.set("extruder", (int) id);
    }
    return true;
}

// Ultra (glTF): per-vertex colours across ALL volumes of the single imported object, concatenated
// in volume order. Same encoding as obj_import_vertex_color_deal, walked with a running offset.
// The total length is checked before anything is painted, so a mismatch leaves the model clean.
bool Model::import_multi_volume_vertex_color_deal(const std::vector<unsigned char> &vertex_filament_ids, const unsigned char &first_extruder_id, Model *model)
{
    if (vertex_filament_ids.empty())
        return false;
    if (model->objects.size() != 1)
    ...
    if (total != vertex_filament_ids.size())
        return false;
    obj->config.set("extruder", first_extruder_id);
    size_t offset = 0;
    for (ModelVolume *volume : obj->volumes) {
        const size_t count = volume->mesh().its.vertices.size();
        volume->config.set("extruder", first_extruder_id);
        if (!paint_volume_from_vertex_colors(volume, vertex_filament_ids.data() + offset, count))
            return false;
        offset += count;
    }
    return true;
