...
void HubServer::handle_hub(tcp::socket& client, Request& r)
{
    if (r.path == "/hub/info" && r.method == "GET") {
        ...
        // P5's stand-in for P4. P4 owns this route and HubServer::accept_event(json&), which
        // assigns the id and the time, keeps a ring of recent events and balloons the tray;
        // when the two branches merge, this block goes and accept_event() calls
        // RemoteNotify::deliver(event) at the marked line instead.
        if (r.content_type.compare(0, 16, "application/json") != 0) { respond_json(client, 415, json_error("Content-Type must be application/json")); return; }
        std::string body;
        if (!read_small_body(client, r, body, 64 * 1024)) { respond_json(client, 413, json_error("that is too large for an event")); return; }
        json event;
        try {
            event = json::parse(body);
        } catch (...) { respond_json(client, 400, json_error("the body must be JSON")); return; }
        if (!event.is_object()) { respond_json(client, 400, json_error("the event must be a JSON object")); return; }
        static std::atomic<long long> s_event_id { 0 };
        event["id"]   = ++s_event_id;
        event["time"] = (long long) std::chrono::duration_cast<std::chrono::milliseconds>(
                            std::chrono::system_clock::now().time_since_epoch()).count();
        RemoteNotify::deliver(event); // queued; the request thread never waits on a relay
        json ok;
        ok["ok"] = true;
        ok["id"] = event["id"];
        respond_json(client, 200, ok.dump());
...
static bool instance_api_allowed(const std::string& method, const std::string& sub)
{
    ...
    // /api/printers/<id>/control - <id> is a Bambu dev_id, the literal "host" / "connect" or a LAN
    // Snapmaker's sm:<id> (hence the colon and the percent escape the page makes of it), so this
