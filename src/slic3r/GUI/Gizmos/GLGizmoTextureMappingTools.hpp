#ifndef slic3r_GLGizmoTextureMappingTools_hpp_
#define slic3r_GLGizmoTextureMappingTools_hpp_
...
class GLMmSegmentationGizmo3DSceneImageMap
{
public:
    GLMmSegmentationGizmo3DSceneImageMap() = delete;

    explicit GLMmSegmentationGizmo3DSceneImageMap(size_t triangle_indices_buffers_count)
    {
    }

    virtual ~GLMmSegmentationGizmo3DSceneImageMap() { release_geometry(); }
...
class GLGizmoMmuSegmentationImageMap : public GLGizmoPainterBase
{
public:
    GLGizmoMmuSegmentationImageMap(GLCanvas3D& parent, const std::string& icon_filename, unsigned int sprite_id);
    ~GLGizmoMmuSegmentationImageMap() override;
...
protected:
    ...
    EnforcerBlockerType get_right_button_state_type() const override { return EnforcerBlockerType(-1); }
    bool should_render_triangle_texture_preview() const override { return true; }
...
private:
    ...
    void mark_solid_color_projection_dirty();
    bool ensure_solid_color_projection_image();
    ...
    void render_solid_color_projection_controls(float max_tooltip_width);
    ...
    OverlayRect default_overlay_rect() const;
    ...
    bool                 m_solid_color_mode = false;
    std::array<float, 3> m_solid_color { 91.f / 255.f, 102.f / 255.f, 1.f };
    float                m_solid_color_aspect = 0.f;
    bool                 m_solid_color_dirty = true;
    std::vector<uint8_t> m_solid_color_rgba;
    uint32_t             m_solid_color_width = 0;
    uint32_t             m_solid_color_height = 0;
    ...
    float                m_projection_scale = 1.f;
    bool                 m_projection_scale_reset_active = false;
    double               m_projection_scale_last_click_time = -1.0;
...
#endif // slic3r_GLGizmoTextureMappingTools_hpp_
