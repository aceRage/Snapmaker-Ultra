...
#if 0
FFConnectPrinterDialog::FFConnectPrinterDialog(wxWindow *parent, wxWindowID id, const wxString &title, const wxPoint &pos, const wxSize &size, long style)
    : DPIDialog(parent, id, _L("ConnectPrinter (LAN)"), pos, size, style)
{
    ...
    m_textCtrl_code->Bind(wxEVT_TEXT, &FFConnectPrinterDialog::on_input_enter, this);
    m_button_confirm->Bind(wxEVT_BUTTON, &FFConnectPrinterDialog::on_button_confirm, this);
    wxGetApp().UpdateDlgDarkUI(this);
}
#endif
FFConnectPrinterDialog::FFConnectPrinterDialog(bool err_hint /*= false*/) 
    : TitleDialog(static_cast<wxWindow *>(wxGetApp().GetMainTopWindow()), _L("Connect Printer (LAN)"), 6)
{
    ...
    this->Fit();
    CentreOnParent();
    Bind(wxEVT_SHOW, &FFConnectPrinterDialog::on_show, this);
    m_textCtrl_code->Bind(wxEVT_TEXT, &FFConnectPrinterDialog::on_input_enter, this);
    m_button_confirm->Bind(wxEVT_LEFT_DOWN, &FFConnectPrinterDialog::on_button_confirm, this);
    wxGetApp().UpdateDlgDarkUI(this);
}

FFConnectPrinterDialog::~FFConnectPrinterDialog() {}

void FFConnectPrinterDialog::on_show(wxShowEvent &event) 
{ 
    Layout(); 
}

void FFConnectPrinterDialog::end_modal(wxStandardID id)
{
    EndModal(id);
}

void FFConnectPrinterDialog::set_machine_object(MachineObject* obj) 
{ 
    m_obj = obj;
}

void FFConnectPrinterDialog::set_device_object(DeviceObject* devObj)
{
    m_devObj = devObj;
}

void FFConnectPrinterDialog::on_input_enter(wxCommandEvent& evt)
{
    m_input_access_code = evt.GetString();
}


void FFConnectPrinterDialog::on_button_confirm(wxMouseEvent &event)
{
...
void FFConnectPrinterDialog::on_dpi_changed(const wxRect &suggested_rect)
{
    m_textCtrl_code->GetTextCtrl()->SetSize(wxSize(-1, FromDIP(22)));
    m_textCtrl_code->GetTextCtrl()->SetMinSize(wxSize(-1, FromDIP(22)));
