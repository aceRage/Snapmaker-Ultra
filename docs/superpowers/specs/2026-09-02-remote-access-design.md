...
`tailscale serve --bg http://127.0.0.1:13640` on the PC (node `reaper`, Tailscale 1.102.3) published `https://reaper.tail2dff02.ts.net/`; the phone on cellular loaded the page over HTTP/2 and the Bambu cameras (go2rtc MSE over `/api/ws`) played, so the WebSocket-over-HTTP/2 concern (tailscale/tailscale#20882) does not reproduce with this stack. Through Serve the hub's control routes answer 404 (the Host check), and a WebSocket upgrade over HTTP/1.1 answers 101. Found on the way: Snapmaker U1 cameras were black remotely because the page embedded the printer's own LAN WebRTC page; the hub now relays the U1's raw H.264 (`/webcam/stream.h264`) into go2rtc from the first SPS (go2rtc misreads a mid-GOP start as HEVC) and the phone plays it through the same tunnel whenever it is not on a LAN http origin. Still to do for Phase 1: `Secure` on the `rt` cookie when served over https, the Serve switch on the hub page, identity allow-list.
