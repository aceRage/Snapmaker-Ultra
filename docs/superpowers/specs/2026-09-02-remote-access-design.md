...
Two things found on the way, both pre-existing and neither addressed here: `shutdown()` deletes `hub.json`, so `remote_on` and the tailnet allow-list do **not** in fact survive a hub restart (1c claims they do) - after a clean quit the next hub starts with remote access off and an empty allow-list while Tailscale keeps serving, which means Serve answers 403 until somebody turns it on again; and on Windows a second hub on another data dir can bind `127.0.0.1:13640` underneath the first hub's `0.0.0.0:13640`, so it silently shadows loopback traffic (the acceptor sets no `SO_EXCLUSIVEADDRUSE`).
