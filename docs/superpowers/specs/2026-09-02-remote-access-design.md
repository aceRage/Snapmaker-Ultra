...
## 1e. Phase 2 status (2026-09-02): DONE, gate held

Shipped on `feat/webrtc-video`: go2rtc gets its WebRTC listener back - `webrtc: listen: ":<port>"` with `ice_servers: ["stun:stun.cloudflare.com:3478"]`, where `<port>` is 8555 (go2rtc's own documented default, so one firewall rule the user allows keeps working across restarts) or the first port up to 8574 that is free for **both** UDP and TCP; on the development PC another go2rtc already held 8555 and the hub took 8556, which is why the port is reported rather than assumed. Everything Phase 0a established is untouched: `local_auth`, the per-run credentials, the random loopback API port and `allow_paths: ["/api/ws", "/api/streams", "/api/onvif"]`. **No new API path was needed.** In go2rtc 1.9.14 the player signals WebRTC as JSON messages (`webrtc/offer`, `webrtc/answer`, `webrtc/candidate`) on the `/api/ws` WebSocket the hub already tunnels; the HTTP `/api/webrtc` route is WHEP, for clients that do not use the WebSocket, and it stays unregistered (the gate checks it still answers 401/404). The hub's `tunnel()` is a byte splice in both directions, so ICE candidates cross it verbatim - the gate asserts that both a LAN host candidate and a Tailscale 100.64/10 candidate arrive unaltered.

The phone's camera tiles now ask the player for `mode=webrtc,mse`. That is not "try one, then the other": go2rtc's player starts MSE and the peer connection together, plays the relayed MSE stream at once, and swaps to WebRTC the moment the peer connection produces a picture (`onpcvideo`), closing the WebSocket as it does. So the picture never waits for ICE and a blocked port costs nothing - MSE simply keeps playing. `player.html` adds a give-up timer (8 s, overridable with `?rtctimeout=`) that closes a peer connection which has not taken over, so the browser stops running an ICE agent and a second decoder behind a picture the viewer can already see. A per-phone setting beside the layout picker (localStorage `snorca_remote_video`) offers **auto / WebRTC / relay**, default auto; auto degrades to plain `mse` when the hub reports no WebRTC port. The PC's own Stream tab keeps `mode=mse`: it reaches go2rtc over loopback through the hub, where DTLS, SRTP and ICE would only add work.

**What a phone on the tailnet actually connects to, and a correction to Part A §5.** go2rtc offers one host candidate per interface (its log lists them at start): the Wi-Fi LAN address, the Tailscale 100.x address and its `fd7a:` IPv6, any VPN adapters, plus one server-reflexive candidate discovered through STUN. A phone on the tailnet can route only to the 100.x / `fd7a:` ones, so **the media rides WireGuard, exactly as the page does**. Part A's claim that WebRTC "removes the risk of stalls when Tailscale falls back to its DERP relays" is therefore wrong for the Serve topology - if the tailnet is relaying, it relays the media too. What Phase 2 does buy is narrower and still real: the media leaves the hub's TCP tunnel and Serve's TLS proxy (two userspace copies per frame), it becomes UDP with congestion control built for video, on the home Wi-Fi it is a genuinely direct path, and for any Phase 3 transport with a bandwidth cap or a video clause (Funnel, zrok, Pinggy) the video never enters the tunnel at all. The STUN server only matters off the tailnet, and without port forwarding its server-reflexive candidate (an ephemeral port, not the media port) will rarely complete - a phone on mobile data with no Tailscale keeps using MSE, which is the intended outcome rather than a failure.
...
**Measurements** (PC-side clients through the hub's own `/api/ws`, `snorca_hubtest\measure_video.py`, aiortc against a hand-written MSE client, 20 s per run, two runs; start-up is from opening the WebSocket to the first media byte or frame, the rate is over the media window only, so no handshake is charged to either):

| stream | path | start-up | rate | notes |
|---|---|---|---|---|
| Snapmaker U1 (h264 relay), 1920x1080 | WebRTC | 1.56 / 2.37 s | 316 / 315 kB/s | 536 / 500 frames, `connected` |
| Snapmaker U1 (h264 relay) | MSE | 1.18 / 1.83 s | 309 / 306 kB/s | `avc1.640029` |
| Bambu X1C (`rtspx`) | WebRTC | 1.70 / 1.77 s | 89 / 106 kB/s | 389 / 459 frames, `connected` |
| Bambu X1C (`rtspx`) | MSE | 1.94 / 1.23 s | 95 / 105 kB/s | `avc1.640029` |

Both paths carry the same H.264 with no transcoding, so the rates differ only by container overhead (RTP/SRTP against fMP4) and sit within a few per cent of each other; start-up differs by about half a second, usually in MSE's favour because it has no ICE and DTLS handshake to do first. **From the PC, WebRTC is neither faster nor cheaper - the whole value is which wire the bytes take**, and that can only be measured from a phone. `mode=webrtc,mse` is what makes the difference invisible: the viewer gets MSE's start-up and WebRTC's path. A real Chrome on the PC confirmed the player end to end - with `mode=webrtc,mse` the overlay reads `RTC`, `pc.connectionState` is `connected`, the WebSocket has closed and the video element plays a `MediaStream` at 1920x1080; with the give-up timer forced short the peer connection is dropped, the overlay reads `MSE`, the WebSocket stays open and the same picture keeps playing from the MediaSource.

**Not verified here, and left for review:** a phone on the home Wi-Fi and a phone on the tailnet. Every WebRTC session measured above ran between two processes on the same PC, so it never crossed Windows Firewall; whether the media reaches a real phone is exactly what the firewall rule in the note decides.

Gate: `snorca_hubtest\test_webrtc.py` against an isolated hub (`--datadir ...\dd_next`, port 13641, its own go2rtc) - the media port is in range, listening, and matches `hub.json`; the config keeps `local_auth` and the unchanged `allow_paths`; `/hub/info` and the phone's `/state` report the port, the firewall verdict and a note exactly when there is something to say; the player gate and the WebSocket tunnel still behave (404 without a gate, 200 and 101 with `lt`, go2rtc's WHEP route still refused); a WebRTC session to a U1 stream and to a Bambu stream reaches `connected` and delivers frames and bytes; LAN and Tailscale candidates survive the tunnel; MSE still delivers on the same streams; and the pages carry the mode wiring. Re-run against the same hub: `test_security_next.py` (Phase 0a, 36 checks) and `test_u1ws_next.py` (U1 relay plus MSE), both PASS.

Still open from the Phase 2 row of the rollout table (Part A §7h): the low-bitrate remote profile, the snapshot-only mode using go2rtc's `/api/frame`, and the byte counter. None of them is needed for WebRTC itself; they are the "make a metered tunnel survivable" half of §7h and belong with Phase 3, when a metered transport exists to survive.
