...
## 1a. Phase 0a status (2026-09-02): DONE, gate held

Both live findings are closed on feat/ultra-preferences; the LAN-only build no longer has a browser-reachable path to command execution or to the hub's control routes.

**F1 (go2rtc).** go2rtc now starts on a random loopback port with per-run `username`/`password`, `local_auth: true` and `allow_paths: ["/api/ws", "/api/streams", "/api/onvif"]`; `origin` is gone and the fixed port 21984 (and the "reuse an orphan" probe) with it. Two facts about go2rtc 1.9.14 differ from the draft in Part C and shaped the implementation: `allow_paths` is a whitelist of the *registered* routes (everything else, the built-in player pages included, answers 404 even with credentials), and `local_auth` makes every remaining path credential-only, WebSocket upgrades included. So the hub is now go2rtc's only client: it registers streams and runs ONVIF discovery with the credentials (URL userinfo through libcurl), serves the player itself (`resources/web/orca/player.html` plus verbatim copies of go2rtc's `video-stream.js` / `video-rtc.js`, refreshed together with go2rtc.exe), and attaches `Authorization` while stripping the client's `Cookie` on the `/api/ws` tunnel. The player is gated by the phone's `rt` cookie or, for the PC's own Stream tab and from loopback only, by the hub secret passed as `lt`; the Stream tab no longer touches go2rtc's API (stream registration rides the pushed state, ONVIF discovery goes page -> slicer -> hub -> go2rtc). The external go2rtc.org icon/manifest links the stock player carried are gone as a side effect.

**F2 (`/hub/*`).** A per-run 128-bit secret is generated at hub start, written to `<datadir>/hub/hub.json` (which the slicer instances already read for the port) and injected into the served hub page; every `/hub/*` request must come from a loopback peer with a loopback `Host` (no DNS rebinding), must not carry `Sec-Fetch-Site: cross-site`, and - the page and `qrcode.js` excepted - must present the secret as `X-Hub-Secret`, a custom header a cross-origin page cannot send without a preflight the hub never answers. `/hub/state` requires `Content-Type: application/json`. The hub page gets `X-Frame-Options: DENY`. Not done here (Phase 0b): the separate loopback admin acceptor, header-read bounds and timeouts, the explicit proxy allow-list.

Gate (`snorca_hubtest\test_security.py` against the real data dir, plus `test_hidden.py` and the visible preview regression): no header / wrong header / non-loopback Host / cross-site / LAN peer -> 403 or 404; page carries the secret; text/plain state -> 415; go2rtc on a random port, 401 without credentials, `/api/config` and `/api/exit` 404 even with them, nothing on 21984; player 404 without cookie or `lt`, 200 with either, `lt` refused from the LAN; WebSocket upgrades through the hub answer 101 with either gate and 404 with neither; `/hub/onvif` relays discovery. Manual check left to the user: cameras still show in the PC's Stream tab and on the phone.

## 1b. Experiment 1 result (2026-09-02): Tailscale Serve carries the phone app, WebSocket included

`tailscale serve --bg http://127.0.0.1:13640` on the PC (the PC node, Tailscale 1.102.3) published `https://<machine>.<tailnet>.ts.net/`; the phone on cellular loaded the page over HTTP/2 and the Bambu cameras (go2rtc MSE over `/api/ws`) played, so the WebSocket-over-HTTP/2 concern (tailscale/tailscale#20882) does not reproduce with this stack. Through Serve the hub's control routes answer 404 (the Host check), and a WebSocket upgrade over HTTP/1.1 answers 101. Found on the way: Snapmaker U1 cameras were black remotely because the page embedded the printer's own LAN WebRTC page; the hub now relays the U1's raw H.264 (`/webcam/stream.h264`) into go2rtc from the first SPS (go2rtc misreads a mid-GOP start as HEVC) and the phone plays it through the same tunnel whenever it is not on a LAN http origin. Still to do for Phase 1: `Secure` on the `rt` cookie when served over https, the Serve switch on the hub page, identity allow-list.

## 1c. Phase 1 status (2026-09-02): DONE, gate held

Shipped on feat/ultra-preferences: the hub detects Tailscale (`tailscale status --json`: backend state, machine DNS name, owner login, certificate domains) and the hub page gains a "Remote access (Tailscale)" card: Turn on runs `tailscale serve --bg --https=443 http://127.0.0.1:<port>` (first run fetches the certificate, up to a minute), shows `https://<machine>.<tailnet>.ts.net/r/<token>/` with a QR, and edits an allow-list of tailnet logins seeded with the owner; Turn off runs `tailscale serve --https=443 off`. Identity: a loopback peer carrying `Tailscale-User-Login` is treated as coming through Serve and refused with 403 unless the login is allow-listed; LAN peers are never loopback so cannot forge it; the LAN token path is unchanged. `rt` is `Secure` when `X-Forwarded-Proto: https`. `remote_on` and the allow-list persist in hub.json; at start the Serve proxy is re-pointed if the hub's port changed. The slicer's phone dialog shows the remote link too.

Deviation from Part A: the hub does not bind loopback-only in remote mode - the forgery concern is met by trusting the identity header from loopback peers only, and the LAN link keeps working for phones on the home Wi-Fi. Not done: tailnet identity replacing the token (the token stays as a second gate on both paths), IPv6 / 100.64/10 direct-connect acceptance (unneeded with Serve), the phone-side "signed in as" indicator.

Gate: `test_remote.py` - detection, on, link, owner seeded, Serve proxy present, page 200 through Serve with a Secure cookie, `/hub/*` still 404 through Serve, owner removed -> 403 -> added back -> 200, last visit recorded, LAN page unchanged with a plain cookie, spoofed header from the LAN ignored / from loopback enforced, hub.json persistence, off removes the proxy, on restores the same link; plus the security and U1 relay gates.

## 1d. Phase 0b status (2026-09-02): DONE, gate held

Shipped on `feat/hub-hardening-0b` (branched from feat/ultra-preferences): the control plane no longer shares a socket with anything a tunnel can front, and nothing a phone sends is unbounded. Part C blockers B2 (the acceptor half), B4, B7, B8, B9 and findings F3, F5, F6, F9, F10 (the header half) are closed; the LAN path, the phone page, hidden instances and Tailscale remote access are unchanged in behaviour.

**Admin listener (B2).** A second acceptor binds `127.0.0.1:0` at hub start and the port it gets is written to `<datadir>/hub/hub.json` as `admin_port`. `/hub/*` and `/relay/h264` answer there and nowhere else; the main listener returns 404 for both, and the admin listener returns 404 for everything else. So a tunnel pointed at the hub port (Tailscale Serve today, zrok or Pinggy later) cannot reach the control routes even though its requests arrive from a loopback peer, and go2rtc's U1 source URL is out of reach as well. The existing gates stay on top of that: loopback peer, loopback `Host`, no `Sec-Fetch-Site: cross-site`, `X-Hub-Secret`. The slicer instances (`hub_call`, `post_state`, `onvif_discover`, `query`, `ensure_running`), the tray's "Open hub page" and the hub page itself now use `admin_port`; a hub.json without one falls back to `port`, which is what lets a running pre-0b hub still be told to quit during an upgrade. `register_streams()` hands go2rtc `http://127.0.0.1:<admin_port>/relay/h264?...`, which also stops those URLs going stale when phone access rebinds the main listener. The phone page, `/stream.html`, `/video-*.js` and the `/api/ws` tunnel stay on the main port: phones use them, and the PC's Stream tab embeds the player with `window.__hubReady(port, relay, secret)` from StreamPanel.cpp, which still passes the main port.

**Request hygiene (B4, F3).** The request head is capped at 16 KiB (`asio::streambuf req(MAX_HEAD)`) and the connection is dropped when it does not fit. Blocking-socket receive deadlines replace the missing timeouts: 10 s for the head, 30 s between reads of a buffered body, 120 s between reads of an upload, and a 30 s send deadline so a peer that stops reading cannot park a thread either. Tunnels (go2rtc WebSocket, the Bambu MJPEG relay, `/relay/h264`) clear the read deadline - a camera may be quiet - and instead get TCP keep-alive with a 30 s idle and 5 s probes, so a phone that walks out of range frees its two threads in about a minute rather than in two hours. Each listener counts its live connections and answers 503 past the cap: 128 on the main listener (a phone with six camera WebSockets plus its polling is nowhere near it) and 64 on the admin listener, which only ever serves this PC. The same bounded-head-plus-deadline treatment is applied to the instance API in `RemoteAccess.cpp`, which the proxy exposes to the phone.

**Token and comparison (B8, F9).** `random_token()` draws its 14 symbols straight from `std::random_device` with rejection sampling instead of seeding an mt19937 with 64 bits; `random_hex()` (the per-run secret and go2rtc's credentials) already did. A `ct_equal` helper compares without an early exit and refuses empty values, and is used for the `X-Hub-Secret` header, the phone token in `/r/<token>/...` (pulled out of the path first, so it is a value comparison rather than a prefix match), the `rt` cookie and the `lt` parameter on the player and the U1 relay.

**Windows Firewall.** The media port is the only inbound port the hub needs, so the hub looks at what Windows allows and says so; it never adds a rule (that needs administrator rights, and doing it silently would be wrong). The query is `Get-NetFirewallApplicationFilter | Get-NetFirewallRule` through PowerShell rather than `netsh advfirewall firewall show rule`, because the cmdlets answer with property values (`Allow`, `Inbound`, `Private`) that are the same in every Windows display language while netsh's verbose output is localised and would have to be parsed by label. It takes about nine seconds, so it runs on a detached thread and is cached for five minutes - which also means the hub page corrects itself once the user has allowed the program. The verdict is `allowed` / `partial` / `missing` / `blocked` / `unknown`, compared against the profiles of the PC's **live** networks, and it appears on the hub page's remote card (with the exact path to paste into the firewall dialog) and, when it is not `allowed`, as a warning glyph beside the phone's Video setting carrying the same sentence. `blocked` is reported ahead of any allow rule because a block rule wins, and Windows writes one whenever its "allow access?" prompt is dismissed - reporting "allowed" while a block rule silently won would be worse than saying nothing. Two observations from the development PC. First, the profile split is real: Wi-Fi is a *Public* network and the Tailscale adapter is *Private*, while the rules Windows had created for the main checkout's `go2rtc.exe` cover *Public* only - so on that install LAN WebRTC would work and tailnet WebRTC would not, which is exactly the half-working state the note exists to explain. Second, the common case resolves itself: when the test hub's go2rtc bound the media port from its new path for the first time, Windows created inbound allow rules for Private and Public by itself, and the hub's verdict moved from `missing` to `allowed` at the next five-minute re-check with the note disappearing from both pages. So most users should never read the note; it is there for the ones whose firewall prompt was dismissed, is managed by policy, or covers the wrong profile.

**Uploads (B7, F5).** An upload folder is now `<stamp>_<8 hex>`, so two uploads in the same second no longer share one (the second used to overwrite the first). A failed upload removes its folder instead of leaving a half file. A 1 GiB total quota over `<datadir>/hub/uploads` is enforced at hub start and after every upload by deleting whole upload folders oldest first, skipping anything younger than an hour (a slicer may still be opening it) and stopping once under the quota; if everything is younger than an hour it logs a warning and deletes nothing. `<datadir>/hub/saves` - the user's own projects - is never touched.

**Spawn cap (B7, F6).** `POST /hub/new` and `POST /r/<token>/api/instances/open` answer 429 with a plain-English message once six instances are in flight, counted as the registered instances plus spawns whose `<pid>.json` has not appeared yet (a fresh instance takes seconds to register, which is long enough for a phone to ask for a dozen more). The phone route refuses before spooling, so a refused request does not write a gigabyte first.

**Proxy allow-list (B9).** `/r/<token>/i/<pid>/api/...` was a catch-all: every route the instance grew was exposed the day it was written. It is now an explicit method+path list mirroring the manifest in `RemoteAccess::handle_api`, with `{index}`/`{id}` segments matched as numbers; anything else answers 404 `route not proxied; see /api`. `/api/debug/*` is deliberately absent. `test_hardening.py` fetches `/api` from a live instance and asserts every manifest route is proxied, so adding a route to the manifest without adding it here fails the gate.

**Response hygiene (F10).** Every response the hub writes carries `X-Content-Type-Options: nosniff` and `Referrer-Policy: no-referrer` (the latter matters because the token is in the path and the page loads printer pages in iframes). The phone page joins the hub page in `X-Frame-Options: DENY`.

Deviations and decisions for review:
- **The player carries no `X-Frame-Options` at all.** `DENY` would break the phone page's own camera tiles, and `SAMEORIGIN` would break the PC's Stream tab, which loads `stream_center.html` from `http://localhost:<page port>` and frames `http://127.0.0.1:<hub port>/stream.html` - a different origin. The player is already gated by the `rt` cookie or a loopback `lt`, so framing it buys an attacker nothing they do not already have.
- **Tunnelled responses are not rewritten.** The go2rtc WebSocket handshake, the Bambu MJPEG stream and the instance API answers pass through untouched (a 101 upgrade is not a place to inject headers). `/relay/h264`, which the hub writes itself, does carry the two headers.
- **Thread-per-connection stays.** Part C suggested an Asio thread pool; the cap plus the deadlines close the same hole with far less surgery, and the hub's peak is a handful of connections.
- **Limits are guesses with headroom**, all named constants at the top of `RemoteHub.cpp`: `MAX_HEAD` 16 KiB, `HEAD_TIMEOUT_S` 10, `BODY_TIMEOUT_S` 30, `UPLOAD_TIMEOUT_S` 120, `WRITE_TIMEOUT_S` 30, `TUNNEL_WRITE_S` 120, `MAX_CONNECTIONS` 128, `MAX_ADMIN_CONNECTIONS` 64, `UPLOAD_QUOTA` 1 GiB, `UPLOAD_MIN_AGE_S` 3600, `MAX_INSTANCES` 6.
- **Over the cap the 503 can be lost.** The refusal is written before the request is read, so closing the socket with the request still in the receive buffer resets the connection and the client may see the reset rather than the 503 (about half the time in the gate, which therefore asks a few times and only insists that nothing is ever *served*). The one-line fix is to move the `guard.ok()` check below `read_request` in `HubServer::serve` - the head read is already bounded and deadlined, so it costs nothing an attacker was not going to spend anyway. Left out here only because installing it means stopping the slicer windows the user had open at the time.
- Still open from Part C and not in this phase: `Host` validation on the *main* listener and the instance API (B3 - only the control routes check it), the `Transfer-Encoding` / method-charset hardening of `force_close` (F8), the absolute-path assertion in `spawn_slicer` (F7), Windows device names in upload file names (`NUL.stl`, F5), opaque upload ids instead of absolute paths in answers (F11), `is_private_v4` accepting 100.64/10 (F12), and the `BambuCamRelay` notes (F13).

Two things found on the way, both pre-existing and neither addressed in this phase; **both are fixed as of 2026-09-11** (see `2026-09-11-hub-remote-persist.md`). First, `shutdown()` deletes `hub.json`, so `remote_on` and the tailnet allow-list did **not** in fact survive a hub restart (1c claims they do) - after a clean quit the next hub started with remote access off and an empty allow-list while Tailscale kept serving, which meant Serve answered 403 until somebody turned it on again. The hub's *runtime record* (pid, port, admin_port, secret - what another process reads to find the live hub) stays in `hub.json` and is still removed on a clean quit; its *settings* (remote_on, the allow-list, the phone token, the notification destinations) moved to `<datadir>/hub/settings.json`, which survives, and an old `hub.json` carrying the two keys is imported once. Second, on Windows a second hub on another data dir could bind `127.0.0.1:13640` underneath the first hub's `0.0.0.0:13640` and silently shadow loopback traffic (the acceptor set no `SO_EXCLUSIVEADDRUSE`); both the main and the admin acceptors now bind exclusively on Windows, so the second hub's bind fails and it moves along the port range with a log line saying another hub is probably already running. Gate: `snorca_hubtest	est_hub_persist.py` (gate_all.sh section `persist`).

Gate: `snorca_hubtest\test_hardening.py`, seven sections, all green against the deployed build - the admin split (`/hub/*` and `/relay/h264` 404 on the main listener and answer on the admin one, the admin port refused from the LAN, the phone and player routes still on the main port, the Phase 0a gates still enforced); request hygiene (a >16 KiB head and a head that never ends are both dropped, 140 held connections make the next ones 503 and never served, the hub recovers); comparison (no prefix, suffix, length or empty short-cut on the secret, the token or `lt`, and no measurable timing difference between a near-miss and a total miss over 160 interleaved requests); the proxy allow-list against the live manifest (26 routes, all proxied; `/api/debug/*`, unknown paths and wrong methods are not - probed with an over-size `Content-Length` so that even `POST /api/quit` can be tested without quitting anything); the headers; and, on a data directory built from scratch under `snorca_hubtest\dd_quota` with a stub instance so that no slicer is spawned, the upload quota at start (1300 -> 900 MiB, the fresh upload kept, the oldest deleted, `hub/saves` untouched), collision-proof folder names (two uploads in the same second, same stamp, different suffix), the post-upload clean-up, and the 429 at six instances on both routes with nothing spooled. Then `test_security.py`, `test_remote.py`, `test_u1ws.py` and `test_hidden.py` all pass unchanged in meaning (their `/hub/*` calls now read `admin_port` from hub.json). Manual check left to the user: cameras in the PC's Stream tab and on the phone.

## 1e. Phase 2 status (2026-09-02): built and gated, parked on `feat/webrtc-video`

WebRTC video (go2rtc `webrtc.listen` on 8555-8574 with a public STUN, phone tiles trying WebRTC then falling back to MSE, a per-phone Video setting, Windows Firewall detection on the hub page) is complete on branch `feat/webrtc-video` with its own gate (`test_webrtc.py`, 33 checks) and measurements: same bytes as MSE, about 0.4 s slower start from the PC, and DERP fallback would carry it too. Kept unmerged by decision: MSE already works on the LAN and through Tailscale Serve; merge it when smoother remote watching on weak links matters or before a capped no-app transport (Phase 3). Expect a small conflict with the Phase 0b admin listener in RemoteHub.cpp at merge time.
