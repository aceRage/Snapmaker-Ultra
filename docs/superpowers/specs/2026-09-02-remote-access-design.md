...
## 1e. Phase 2 status (2026-09-12): brought into the tree on `feat/webrtc-video-2`

WebRTC video was built and gated on 2026-09-02 (branch `feat/webrtc-video`) and parked unmerged: MSE already worked, so the direct path was not worth the merge risk at the time. It has now been re-applied onto `feat/ultra-preferences` as `feat/webrtc-video-2`, together with a per-phone Quality setting. The predicted conflict with the Phase 0b admin listener showed up exactly where this note said it would, plus three more the branch could not have foreseen (the per-tile mode picker, the two-row topbar, the aspect reporter). See **docs/superpowers/specs/2026-09-12-webrtc-video.md** for what changed, how each conflict was resolved, and the re-taken measurements.
