...
## 2a. Stage 1 status (2026-09-02): DONE, gate held

Implemented on feat/ultra-preferences as specified in Phase 1 (all 11 changes plus the Explorer-forwarding tweak). Gate run with `test_hidden.py` (short-path helper dir): hidden instance registered in 15 s with no top-level window, `/info` `/presets` `/settings/process` `/plates` `/plates/{i}/layout` answered, hub Show/Hide/Quit worked and the hub list followed. Findings: (1) `POST /api/objects/transform` returns 200 but moves nothing on a hidden instance because the selection works on GLVolumes that `reload_scene` never built - exactly Phase 2's finding, so it moves from the Stage 1 gate to the Stage 2 gate; (2) the hub relay must send a POST body: `Http::post` only sets the POST fields when a body is present, otherwise libcurl reads the body from stdin and the hub faults (fixed with a dummy form body); (3) an instance takes 30-60 s to exit after `/api/quit` (network agent teardown), the instance file disappears first; (4) thumbnail.png answers 200 with a 2 KB blank image on a hidden instance - Phase 2's silent failure mode, assert on pixels.

## 2b. Stage 2 status (2026-09-02): DONE, gate held

Implemented as specified in Phase 2: `GLCanvas3D::ensure_gl_ready()` (+ `Plater::ensure_gl_ready()`), the warm-up at the top of `GUI_App::post_init` for hub-managed instances (with a bisect knob `SNORCA_HIDDEN_WARMUP=none|gl|full`, default full), `Plater::priv::set_current_panel` no longer bails when not on screen for hub-managed instances, make-current guards in both non-delegating `render_thumbnail` overloads, `load_gcode_preview`, `reset_gcode_toolpaths` (now out of line) and `_update_imgui_select_plate_toolbar`, route guards in `api_plate_thumbnail` and `ensure_preview_loaded`, and `init_gl(popup_error = !hub_managed)`. Gate (`test_stage2.py`): thumbnails for all 6 plates non-blank on a never-shown instance and byte-identical to the visible control on plates 1-3 (plate 0 is the current plate, re-rendered on demand, within 5%; plates 4-5 empty and blank on both), object move takes effect, slice from the phone (36 s), preview info + frames front/left/rear/right with zoom 2 and 3, PC layer range follows, re-slice yields a new result, Show works and the preview holds; idle CPU of a hidden instance 0.4% of one core (R5 needs no short-circuit).

Findings: (1) the first build crashed ~10 s after the warm-up with a symbolised stack (minidbg) `on_idle -> NotificationManager::update_notifications -> PopNotification::count_spaces -> ImGui::CalcTextSize -> ImFont::CalcTextSizeA` reading a null font: `init()` flips `m_initialized`, `on_idle` then measures text with ImGui, but the font atlas is only built by `imgui->new_frame()`, which on the desktop `render()` always calls first. Fix: `ensure_gl_ready()` ends with one empty `new_frame()` + `render()`. (2) The never-laid-out View3D canvas measures 20x0; the offscreen paths set their own viewports so it does not matter, and ImGui gets a 20x10 display size. (3) `wglMakeCurrent` on the never-shown canvas, `init_gl` and FBO rendering all work (R1/R2 settled by the identical thumbnails). (4) The instance log only records warnings, so warm-up milestones are logged at warning level. (5) Visible-window regression done: slice, preview frames, zoom and status routes on a visible instance of this build pass unchanged (test_preview2.py), and quitting a hidden instance leaves a visible one running.

## 2c. Stage 3 status (2026-09-02): DONE, gate held

Implemented: `RemoteAccess::dialog_mode()` (Interactive / Request / Background), a `wxModalDialogHook` installed in `GUI_Run` before the GUI_App exists that answers every `ShowModal()` while hidden or serving a request (class-name rules for input-needing / destructive / print dialogs -> cancel, otherwise style-derived defaults: affirmative in Request mode, do-nothing in Background mode), the fork's own `MessageDialog` / `RichMessageDialog` / `ErrorDialog` / `UnsavedChangesDialog` / `show_error` overrides widened to the same modes and logging to the attention ring, `RemoteAccess::raise_attention/clear_attention/note_attention` with kinds (dialog / timeout / manual), a 1 s GUI heartbeat that reports let-through modals and a stall, `run_on_main` raising a timeout attention (which shows the window) and clearing it after the next completed request, `/api/info` fields `needs_attention` `attention_reason` `attention_kind` `attention_since` `modal_open` `gui_stall_ms` `attention[]`, `POST /api/attention/clear`, the hub list and tray carry the flag, the phone Prepare tab shows a banner with Dismiss, the slice request dispatches the toolbar event synchronously inside the request scope, `api_process_save` passes an explicit name so `SavePresetDialog` is never built, and call-site guards: memory warning (stop + attention), restore-project prompt (skip, keep the backup), force-upgrade (skip + attention), new-version notice, network plug-in update/install, first-run wizard (skip + attention), TLS cert-store prompt (accept while hidden), `SecondaryCheckDialog` / `PrintErrorDialog` `on_show` (not shown, logged). Debug routes `POST /api/debug/{sleep,modal,file}` exist only with `SNORCA_DEBUG_ROUTES=1`.
