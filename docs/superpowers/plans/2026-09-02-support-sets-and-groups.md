...
## 2b. Stage 2 status (2026-09-03): gate replaced and green; T3-T5 done, T6-T7 not started
...
the new gate, its calibration, and where Stage 2 got to.
...
**Gate result: green.** Candidate (T4) against the pre-T4 baseline, 3 passes × 8 cases, all cores,
all within tolerance. That is also the proof that T4 changed nothing in the output.

### What shipped

- **T3** - the harness and corpus, as above, plus `tests/slice_compare_cli`. Corpus cases:
  normal grid / snug / ledge, a dense interface, a soluble interface, organic tree, a raft, and a
  no-support control, over two ~1.2 KB generated fixtures.
- **T4** - the `support_group` key (in `print_config_def`, a member of no static config class and
  ...
  `tests/fff_print/test_support_groups.cpp` - 8 cases, 44 assertions, all pass.
- **T5** - **partly done, and the shortfall is deliberate.** See below.
...
1. **`part_support_keys()` lives in `PrintConfig`, not on `PrintObject`** (§3.5 implied the
   ...
   it belongs; `Model.hpp` already includes `PrintConfig.hpp`.
...
4. **The group name rule applies to group 0 too**, per §3.4 step 3 ("the first non-empty name seen
   among its members"). Two named parts whose values match the object's collapse into the default
   group and it takes the first of their names.

### T5: what is NOT proven

**The 3MF round trip for group data is unverified.** Both routes failed:

- A hand-written fixture did not load. A plain core-spec 3MF built from `<component>`s is imported
  as one `ModelObject` **per component**, and per-part settings keyed on the assembly object id
  *and* on the component object id were both ignored by the importer.
- `store_3mf` is not a way round it: it takes the config by reference and dereferences it
  unconditionally, then walks plate data a bare `Model` does not have, so a unit-test round trip
  through it segfaults. (The other tests in `test_3mf.cpp` pass `nullptr` and only check meshes.)

Rather than leave false coverage, the corpus case and the test were **removed**. Group data is
therefore covered only by the unit tests, which build multi-volume objects with `add_volume`.
**Closing this needs a fixture produced BY the slicer** - save a project with a group from the GUI
once T6 exists, commit that - which is the natural order anyway.

Also unverified: the undo/redo check and the stock-Orca degradation check the T5 brief lists. The
degradation *mechanism* is asserted (`handle_legacy` blanks an unknown key), but no stock build was
run.

### T6 and T7: not started

The Object-List "Support group" submenu, `is_improper_category`, `get_options`,
`PART_CATEGORY_SETTINGS`, `TabPrintPart`, the badge and `SupportGroupsDialog` are untouched. Risks
R2.1-R2.4 belong to them and remain undecided.

### Manual checklist

Nothing user-visible shipped in Stage 2 - the key, resolver and invalidation have no UI yet, and
the generator still ignores groups. To re-run the gate:

```
cmake --install <build> --config Release --prefix <scratch>      # candidate
python scripts/support_group_identity.py \
       --baseline <pre-change exe> --candidate <scratch>/snapmaker-orca.exe \
       --out <scratch-out> --datadir <isolated dd> --continue-on-diff
```

Both test suites: `fff_print_tests "[SupportGroups]"` (8 cases, 44 assertions) and the full
`libslic3r_tests`. Note that `test_mixed_filament.cpp` has two **order-dependent** failures that
predate this work: the suite is green in default order and under seed 1111, and shows one or two
failures under some random seeds. Worth a separate fix; it is a test-isolation problem, not a
product one.
