...
1b. Adopt the tolerant-diff recipe from the base-pipeline non-determinism investigation
   (C:\Dev\SnapmakerOrcaSupports\solid_infill_nondeterminism.md — address-layout knife
   edge on a degenerate solid-infill sliver, NOT a race, each variant byte-stable): in
